...
from django.db.models import Avg, Max, Min, Sum,StdDev,Variance, Value
import statistics
...
def get_map_json_statistics(request, **kwargs):
    data_result = {}
    ...
    for location in locations:
        ...
        # loop para encontrar StdDev y varianaza de cada agrupacion de valores
        values =locationData.values_list('values')
        
        stdDevList=[]
        varianceList=[]
        for value in values:
            stdDevList.append(statistics.stdev(value[0]))
            varianceList.append(statistics.variance(value[0]))
        

        minVal = locationData.aggregate(Min("min_value"))["min_value__min"]
        maxVal = locationData.aggregate(Max("max_value"))["max_value__max"]
        avgVal = locationData.aggregate(Avg("avg_value"))["avg_value__avg"]
        stdDevVal= statistics.mean(stdDevList)
        varVal= statistics.mean(varianceList)
        ...
        data.append(
            {
                ...
                "lng": location.lng,
                "population": stations.count(),
                'std-dev':round(stdDevVal if stdDevVal != None else 0, 6),
                'variance':round(varVal if varVal != None else 0, 6),
                "min": minVal if minVal != None else 0,
                "max": maxVal if maxVal != None else 0,
                "avg": round(avgVal if avgVal != None else 0, 2),
